...
namespace HepMC {
    ...
    {
	if ( (m_mode&HepIOS::out && m_mode&HepIOS::in) ||
	     (m_mode&HepIOS::app && m_mode&HepIOS::in) ) {
	...
	m_file.setf(HepIOS::dec,HepIOS::basefield);
	m_file.setf(HepIOS::scientific,HepIOS::floatfield);
    }
	...
	ostr << "IO_Ascii: unformated ascii file IO for machine reading.\n" 
	     ...
	     << " bad:" << (m_file.rdstate()&HepIOS::badbit)
	     << " eof:" << (m_file.rdstate()&HepIOS::eofbit)
	     << " fail:" << (m_file.rdstate()&HepIOS::failbit)
	     << " good:" << (m_file.rdstate()&HepIOS::goodbit) << std::endl;
    }
	...
	if ( !m_finished_first_event_io ) {
	    ...
	    m_file << "\n" ;
	    m_file << "HepMC::IO_Ascii-legend\n";
	    m_file << " Event line begins with E and contains\n";
	    m_file << "  event_number event_scale alphaQCD alphaQED"
	           << " signal_process_id signal_process_vertex num_vertices"
		   << " random_size random_info\n";
	    m_file << " Vertex line begins with V and contains\n";
	    m_file << "  barcode id x y z t num_orphans_in"
	           << " particles_out_size num_wgt weight_info\n";
	    m_file << " Particle line begins with P and contains\n";
	    m_file << "  barcode pdg_id px py pz E status theta phi"
	           << " end_vertex flow\n";
